...
int main() {
    ...
    // Array zur Speicherung der Monatsnamen
    const char* monate[] = {
        "Ungültig", "Januar", "Februar", "März", "April", "Mai", "Juni",
    ...
    for (int i = 0; i < nutzerZahl; i++) {
        ...
        while (1) {
            ...
            if (scanf("%d", &monat) == 1 && monat >= 1 && monat <= 12) {
    ...
    for (int i = 0; i < nutzerZahl; i++) {
        ...
        // Monat aus dem Array in Textform ausgeben
        printf("Nutzer %d: %02d. %s %d\n", i + 1, tag, monate[monat], jahr);
    }
